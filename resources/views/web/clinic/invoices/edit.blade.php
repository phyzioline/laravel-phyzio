...
@section('header_title', __('Edit Invoice'))
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card border-0 shadow-sm" style="border-radius: 15px;">
            <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                <h5 class="card-title font-weight-bold mb-0">{{ __('Edit Invoice') }}</h5>
                <span class="badge bg-light text-dark">{{ $invoice->invoice_number }}</span>
            </div>
            <div class="card-body px-4">
                @if($invoice->status === 'paid')
                <div class="alert alert-warning">
                    <i class="las la-exclamation-circle"></i> {{ __('This invoice is fully paid and cannot be edited.') }}
                </div>
                <div class="text-center mt-3">
                    <a href="{{ route('clinic.invoices.show', $invoice->id) }}" class="btn btn-secondary">
                        {{ __('Go Back') }}
                    </a>
                </div>
                @else
                <form action="{{ route('clinic.invoices.update', $invoice->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        <label class="form-label">{{ __('Treatment Plan / Description') }} <span class="text-danger">*</span></label>
                        <input type="text" name="treatment_plan" class="form-control @error('treatment_plan') is-invalid @enderror" 
                               value="{{ old('treatment_plan', $invoice->treatment_plan) }}" placeholder="e.g. 12 Physical Therapy Sessions" required>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="total_cost" id="total_cost" step="0.01" min="0" 
                                   ...
                                   value="{{ old('total_cost', $invoice->total_cost) }}" required>
                            @error('total_cost')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="alert alert-info py-2 small">
                        <i class="las la-info-circle"></i> {{ __('Total Paid') }}: {{ number_format($invoice->total_paid, 2) }} {{ __('EGP') }}
                        <br>
                        <i class="las la-calculator"></i> {{ __('New Final Amount will be calculated as: Total Cost - Discount') }}
                    </div>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <button type="submit" class="btn btn-primary px-5">
                            <i class="las la-save"></i> {{ __('Update Invoice') }}
                        </button>
                    </div>
                </form>
                @endif
