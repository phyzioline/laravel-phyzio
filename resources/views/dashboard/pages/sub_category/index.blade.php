                    ...
                    <div class="card">
                        <div class="add d-flex justify-content-end p-2">
                            @can('sub_categories-create')
                                <a href="{{ route('dashboard.sub_categories.create') }}" class="btn btn-primary">
                                    <i class="fas fa-add"></i> {{ __('Add Sub Category') }}
                                </a>
                            @endcan
                        </div>
                        <div class="card-body">
                            <div class="table-responsive text-center">
                                <table id="example2" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @forelse($data as $sub_category)
                                            <tr>
                                                <td>{{ $loop->index + 1 }}</td>
                                                <td>{{ $sub_category->category->{'name_' . app()->getLocale()} }}</td>
                                                <td>{{ $sub_category->{'name_' . app()->getLocale()} }}</td>
                                                ...
                                                <td>
                                                    ...
                                                    @can('categories-update')
                                                        <a href="{{ route('dashboard.sub_categories.edit', $sub_category) }}"
                ...
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.action = '{{ url('/dashboard/sub_categories') }}/' + id;
                            form.method = 'POST';
