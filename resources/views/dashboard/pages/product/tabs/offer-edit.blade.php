        ...
        <div class="col-md-4">
            ...
            <div class="input-group">
                ...
                <input type="number" step="0.01" class="form-control" name="compare_at_price" 
                       placeholder="0.00" value="{{ old('compare_at_price', $product->compare_at_price ?? '') }}">
            </div>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            ...
            <input type="number" class="form-control" name="amount" 
                   placeholder="{{ __('Enter Amount') }}" value="{{ old('amount', $product->amount ?? 0) }}" required min="0">
            @error('amount')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        <div class="col-md-4">
            ...
            <input type="number" class="form-control" name="min_quantity" 
                   placeholder="1" value="{{ old('min_quantity', $product->min_quantity ?? 1) }}" min="1">
        ...
        <div class="col-md-4">
            ...
            <input type="number" class="form-control" name="max_quantity" 
                   placeholder="{{ __('No limit') }}" value="{{ old('max_quantity', $product->max_quantity ?? '') }}" min="1">
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="track_inventory" id="track_inventory" value="1" {{ old('track_inventory', $product->track_inventory ?? true) ? 'checked' : '' }}>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="sku" 
                   placeholder="{{ __('Auto-generated if empty') }}" value="{{ old('sku', $product->sku) }}" readonly>
            <small class="text-muted">{{ __('Automatically generated') }}</small>
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ __('Barcode') }}</label>
            <input type="text" class="form-control" name="barcode" 
                   placeholder="{{ __('Enter barcode') }}" value="{{ old('barcode', $product->barcode ?? '') }}">
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ __('EAN') }}</label>
            <input type="text" class="form-control" name="ean" 
                   placeholder="{{ __('European Article Number') }}" value="{{ old('ean', $product->ean ?? '') }}">
        </div>
    </div>
    <div class="row mb-3">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="upc" 
                   placeholder="{{ __('Universal Product Code') }}" value="{{ old('upc', $product->upc ?? '') }}">
        </div>
    </div>
</div>
