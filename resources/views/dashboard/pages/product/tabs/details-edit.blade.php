    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" name="product_name_en" 
                   placeholder="{{ __('Enter Name English') }}" value="{{ old('product_name_en', $product->product_name_en) }}" required>
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" name="product_name_ar" 
                   placeholder="{{ __('Enter Name Arabic') }}" value="{{ old('product_name_ar', $product->product_name_ar) }}" required>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control" name="brand_name" 
                   placeholder="{{ __('e.g., Sony, Nike') }}" value="{{ old('brand_name', $product->brand_name ?? '') }}">
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ __('Model Number') }}</label>
            <input type="text" class="form-control" name="model_number" 
                   placeholder="{{ __('e.g., RXZER23') }}" value="{{ old('model_number', $product->model_number ?? '') }}">
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ __('Manufacturer') }}</label>
            <input type="text" class="form-control" name="manufacturer" 
                   placeholder="{{ __('e.g., Nike, Procter & Gamble') }}" value="{{ old('manufacturer', $product->manufacturer ?? '') }}">
        </div>
    </div>
</div>
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <textarea class="form-control" name="short_description_en" rows="3" 
                      placeholder="{{ __('Enter Short Description English') }}">{{ old('short_description_en', $product->short_description_en) }}</textarea>
        ...
        <div class="col-md-6">
            ...
            <textarea class="form-control" name="short_description_ar" rows="3" 
                      placeholder="{{ __('Enter Short Description Arabic') }}">{{ old('short_description_ar', $product->short_description_ar) }}</textarea>
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <textarea class="form-control" name="long_description_en" rows="5" 
                      placeholder="{{ __('Enter Long Description English') }}">{{ old('long_description_en', $product->long_description_en) }}</textarea>
        ...
        <div class="col-md-6">
            ...
            <textarea class="form-control" name="long_description_ar" rows="5" 
                      placeholder="{{ __('Enter Long Description Arabic') }}">{{ old('long_description_ar', $product->long_description_ar) }}</textarea>
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            ...
            <textarea class="form-control" name="bullet_points" rows="4" 
                      placeholder="{{ __('Enter key features, one per line') }}">{{ old('bullet_points', $product->bullet_points ?? '') }}</textarea>
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" name="generic_keywords" 
                   placeholder="{{ __('e.g., Water sport shoes; Electric; Wi-Fi') }}" value="{{ old('generic_keywords', $product->generic_keywords ?? '') }}">
            <small class="text-muted">{{ __('Separate keywords with semicolons') }}</small>
        </div>
        <div class="col-md-6">
            <label class="form-label">{{ __('Product Type') }}</label>
            <input type="text" class="form-control" name="product_type" 
                   placeholder="{{ __('e.g., PORTABLE ELECTRONIC DEVICE STAND') }}" value="{{ old('product_type', $product->product_type ?? '') }}">
        </div>
    </div>
    <div class="row mb-3">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            ...
            <select class="form-select" name="status" id="status" required>
                <option value="inactive" {{ old('status', $product->status) == 'inactive' ? 'selected' : '' }}>{{ __('Inactive (Draft)') }}</option>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="engineer_required" id="engineer_optional" value="0" {{ old('engineer_required', $product->engineer_required ?? 0) == 0 ? 'checked' : '' }}>
                    <label class="form-check-label" for="engineer_optional">
                        <strong>{{ __('Optional') }}</strong> - {{ __('Customer can choose') }}
                    </label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="engineer_required" id="engineer_mandatory" value="1" {{ old('engineer_required', $product->engineer_required ?? 0) == 1 ? 'checked' : '' }}>
